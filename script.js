...
// Música: se reproduce solo después de hacer clic, como exigen Chrome/Edge/Android.
...
function prepareMusic(){
  ...
  // Recarga la pista para evitar fallos cuando se abre desde una carpeta local o desde Live Server.
  if(!musicReady){
    bgMusic.load();
    musicReady = true;
  }
...
async function playMusic(){
  ...
  try{
    ...
    alert('No se pudo reproducir la música. Verifica que el archivo audio/musica.mp3 esté en la carpeta del aplicativo y abre el proyecto con Live Server o en un navegador moderno.');
  }
}
...
document.getElementById('btnStart').addEventListener('click', () => { launchConfetti(); startAutoPlay(); playMusic(); scrollToSection('presentacion'); });
